let app = new Vue({
    ...
    beforeUpdate () {
        ...
        for (i = 0; i < this.list.length; i++) {
            if (this.list[i].hp <= 0) {
                for(let j=0; j <this.monsterBook.length; j++ ){ //잡은 몬스터가 무엇이고 뭔지,경험치와 돈 추가
                    if ( this.list[i].monsterId === this.monsterBook[j].monsterId ){
                    this.characterStatus.currentLevelBar += this.monsterBook[j].exp //list에는 exp가 없는 몬스터가 있음 그래서 북을 사용
        ...
        for(;this.characterStatus.currentLevelBar >= this.characterStatus.totalLevelBar;){
          ...
          //레벨이 올라갈때 마다 캐릭터 상태 정보를 업데이트 가능
